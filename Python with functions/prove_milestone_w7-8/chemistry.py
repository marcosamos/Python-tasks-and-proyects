...
def main():
    ...
    parse_formula(molecular_formula,table)
    # Call the compute_molar_mass function to compute the
    # molar mass of the molecule from the compound list.
    total_mass = compute_molar_mass(parse_formula(molecular_formula,table),table)
    # Compute the number of moles of the sample.
    number_moles = mass_grams / total_mass
    # Print the molar mass.
    print(f"{total_mass:.5f} grams/mole")
    # Print the number of moles.
    print(f"{number_moles:.5f} moles")
    # Call the molecules_dict
    #store in a variable
    molecules_dict = known_molecules_dict()
    # Call the get_formula_name
    # print the result
    Print_formula = get_formula_name(molecular_formula,molecules_dict)
    print(f"Formula's name: {Print_formula}")
    # Sum of protons
    total_protons = sum_protons(parse_formula(molecular_formula,table),table)
    print(f"Sum protons: {total_protons}") 
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for element in symbol_quantity_list:
        ...
        for key, element_list in periodic_table_dict.items():
            
            if symbol == key:
                atomic_mass = element_list[ATOMIC_MASS_INDEX]

                sub_total = atomic_mass * quantity
                total_mass += sub_total
    return total_mass         
                
def known_molecules_dict():  
...
def get_formula_name(formula, known_molecules_dict):
    ...
    for key,molecules_list in known_molecules_dict.items():
        if key == formula:
            name_chemical_formula = molecules_list[0]
            return name_chemical_formula
